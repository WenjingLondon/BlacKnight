...
beforeEach(async () => {
  ...
  const MockStrategyFactory = await ethers.getContractFactory("MockStrategyFactory");
...
console.log('Aggregator deployed at', aggregator.address);
  ...
  await token.connect(user).approve(aggregator.target, ethers.parseEther("1000"));
});

  it("user can withdraw via strategy", async () => {
    // 用户先存款
    await aggregator.connect(user).deposit(token.target, ethers.parseEther("100"));

    // 用户进行提现
    await aggregator.connect(user).withdraw(token.target, ethers.parseEther("50"));
